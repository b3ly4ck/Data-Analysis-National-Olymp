...
for specialty in specialties:
    ...
    c1 = spearmanr(temp_df["elderly"], temp_df["expected_salary"])[0]
    fig = sns.regplot(y="expected_salary", x="age", data=temp_df, x_estimator=np.mean, order=2)
    fig = ax.get_figure()
    plt.ylabel('Зарплата, руб.')
    plt.suptitle(f'{str(round(p, 3))} {str(round(c1, 3))} "{specialty}"')

    fig.savefig("charts/Специализации_регрессия/" + specialty + '.png', transparent=True)